...
# Jaccard or Sorensen groups into replacement and richness/abundance difference 
# (or nestedness) components, for species presence-absence or abundance data.
beta.div.comp <- function(mat, coef="BS", quant=TRUE, save.abc=FALSE)
  #
  # Description --# 
  # Podani-family and Baselga-family decompositions of the Jaccard and Sorensen 
...
# coef: Family of coefficients to be computed --#  "S" or "Sorensen": Podani family, 
#         Sorensen-based indices
#  "J" or "Jaccard" : Podani family, Jaccard-based indices
#  "BS" : Baselga family, Sorensen-based indices
#  "BJ" : Baselga family, Jaccard-based indices
#  "N" : Podani & Schmera (2011) relativized nestedness index.
#  The quantitative form in Sorensen family is the percentage difference.
...
# Jaccard=(b+c)/(a+b+c) and Sorensen=(b+c)/(2*a+b+c) with usual abc notation.
#
...
#  1. Total beta div. = sum(D.ij)/(n*(n-1)) (Legendre & De C·ceres 2013)
...
# inEcology and Evolution, 4, 552-557.
#
...
# atheoretical and empirical comparison of novel approaches. Oikos, 122, 
# 825-834.
#
# Legendre, P. (2014) Interpreting the replacement and richness difference 
# componentsof beta diversity. Global Ecology and Biogeography, 23, xxx-xxx.
#
# Podani, J., Ricotta, C. & Schmera, D. (2013) A general framework for analyzing 
# betadiversity, nestedness and related community-level phenomena based on 
# abundancedata. Ecological Complexity, 15, 52-61.
#
# Podani, J. & Schmera, D. (2011) A new conceptual and methodological framework 
# forexploring and explaining pattern in presence-absence data. Oikos, 120, 
# 1625-1638.
#
# License: GPL-2 
# Author:: Pierre Legendre
{
  ...
  #
  if(!quant) { # Binary dataprovided, or make the data binary
    ...
    if(coef==1|| coef==2) {
      ...
      if(coef==1) { # S¯rensen-based components
        repl <- repl/(2*a+b+c)
        rich <- rich/(2*a+b+c)
        D <- (b+c)/(2*a+b+c)
    ...
    } else if(coef==3) { # Baselga 2010components based on S¯rensen
      D <- (b+c)/(2*a+b+c) # S¯rensen dissimilarity
      repl <- min.bc/(a+min.bc) # replacement, turnover
      rich <- D-repl # nestedness-resultant dissimilarity
    } else if(coef==4) { # Baselga 2012components based on Jaccard
    ...
    #
    # Add the denominators
    if(coef==1) { # S¯rensen-based (% difference) components
      for(i in 2:n) {
        for(j in 1:(i-1)) {
          # Baselga 2013notation:
      ...
      for(i in 2:n) {
        for(j in 1:(i-1)) { # Baselga 2013notation:
    ...
    total.div <- sum(D)/(n*(n-1))
    part <- c(total.div,repl.div,rich.div,repl.div/total.div,rich.div/total.div)
    turn_perc <- repl.div/total.div
    nest_perc <- rich.div/total.div
    #
    res <- list(repl=repl, rich=rich, D=D, part=part, turn_perc=turn_perc, 
                nest_perc=nest_perc, Note=form)
  }
  res
}
